...
def comprehensive_evaluation(model, test_gen, class_names, model_name):
    ...
    detailed_metrics.append({
        ...
        'Recall': recall_micro,
        'F1-Score': f1_micro,
        'Sensitivity': accuracy,  # For multiclass, micro-avg sensitivity = accuracy
        'Specificity': np.nan,    # Not meaningful for micro average
        'AUC-ROC': np.nan,        # Not meaningful for micro average
        'Support': np.sum(support)
    })
...
detailed_metrics, confusion_mat, auc_scores = comprehensive_evaluation(
    alexnet_model, test_gen, class_names, "AlexNet"
)
